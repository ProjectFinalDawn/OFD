...
public class ShootBehaviour : GenericBehaviour
{
	...
	private void Update()
	{
        ...
        if (Input.GetAxisRaw(shootButton) != 0 && !isShooting && activeWeapon > 0 && burstShotCount == 0)
        {
            ...
            {
                Debug.LogWarning($"Shooting encountered an issue: {ex.Message}. Continuing firing.");
            }
        }
    ...
    private void ShootWeapon(int weapon, bool firstShot = true)
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 500f, shotMask))
        {
            if (hit.collider.transform != transform)
            {
                ...
                // Handle shot effects on the target.
                DrawShoot(hit.point, hit.normal, hit.collider.transform, !isOrganic, !isOrganic);

                // Reduce health if the target has a BasicHealth component.
                // Reduce health if the target has a BasicHealth component.
                if (hit.collider != null)
                {
                    BasicHealth health = hit.collider.GetComponent<BasicHealth>();
                    if (health != null)
                    {
                        health.TakeDamage(weapons[weapon].bulletDamage);
                    }
                }
                else
                {
                    Debug.LogWarning("Hit detected, but target collider is null. Skipping damage application.");
                }

                /*else
                {
                    Debug.LogWarning($"Target hit, but no BasicHealth component found on {hit.collider?.name ?? "Unknown Object"}.");
                }*/
            }
        }
    ...
    bool placeSparks = true, bool placeBulletHole = true)
    {
        if (gunMuzzle == null)
        {
            Debug.LogWarning("Gun muzzle is null, skipping DrawShoot.");
            return;
        }
        ...
        if (placeBulletHole && bulletHole != null)
        {
            ...
            else
            {
                ...
                bulletHoleSlot = (bulletHoleSlot + 1) % maxBulletHoles;
            }
    ...
    private void ShotDecay()
    {
        ...
        if (shotDecay > 0.2)
        {
            ...
            if (shotDecay <= 0.4f)
            {
                ...
                if (activeWeapon > 0 && weapons[activeWeapon] != null)
                {
